...
function make_toolbar_bangumi(main_class_name, sub_class_names) {  // class-3
    ...
    return '' +
        `<div class="${main_class_name}">
            ...
            .${main_class_name} .more:hover .more-ops-list {
              display: block
            }

            .${main_class_name}:after {
              display: block;
              content: "";
              clear: both
            }
...
function initToolbar() {
    if (!!$('#arc_toolbar_report')[0]) {
        ...
        const span_class_test = toolbar_obj.children().eq(0).children().eq(1).attr('class')
